...
void calculateChannelValue(Channel &channel)
{
    ...
    for (int i = 0; i < channel.presetCount; i++)
    {
        ...
        if (modes[preset.modeID].currentState > 0)
        {
            ...
            scaledIntensity = presetIntensity * modeIntensity / 255;
            ...
            if (preset.mode == PresetModes::Blink)
            {
                ...
                {
                    presetActive = isPresetActive(preset.priorityMode, targetValue, scaledIntensity);
                }
                else
                {
                    presetActive = false;
    ...
    bool activated = highestActivatedPresetIndex > highestDeactivatedPresetIndex;
    int16_t presetIndex = activated ? highestActivatedPresetIndex : highestDeactivatedPresetIndex;
